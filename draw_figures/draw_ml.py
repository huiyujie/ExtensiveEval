...
from draw_figures.draw_function import draw_one_sys_xgb, draw_combined_anova_ML, draw_multiple_systes,  draw_multiple_systes_xgb
...
if __name__ == "__main__":
    ...
    for sys_conf in util_func.GOOD_SYS:
        draw_one_sys_xgb(sys_conf, util_func.get_factor_names(sys_conf), "good", save=True)
        # draw_combined_anova_ML(sys_conf, "good", "random", save=True)
    for sys_conf in util_func.BAD_SYS:
        draw_one_sys_xgb(sys_conf, util_func.get_factor_names(sys_conf), "bad", save=True)
        # draw_combined_anova_ML(sys_conf, "bad", "random", save=True)
    
#     draw_one_sys_xgb(("silo","ycsb"), util_func.get_factor_names(("silo","ycsb")), "bad", save=True)
