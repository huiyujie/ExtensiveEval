...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), path_to_sample_dir))
)
import sample
...
def check_consistent(model, terms):
    ...
    parms_in_model = (
        cof_table["index"].apply(lambda x: re.sub("\[(T\.)?[^:]+\]", "", x)).unique()
    )
...
def ANOVA_coef_helper(
    ...
    if not removed_terms:
        ...
        df_removed_terms = pd.DataFrame.from_records(
            list(removed_terms.items()), columns=["term", "p-value"]
        )
    ...
    cof_table = model.summary2().tables[1]["Coef."].drop(["Intercept"])
    cof_table = cof_table.reset_index()
    cof_table["parms"] = cof_table["index"].apply(
        lambda x: re.sub("\[(T\.)?[^:]+\]", "", x)
    )
    anova_table = sm.stats.anova_lm(model, typ=3)
    anova_table["per_var_explained"] = (
        anova_table["sum_sq"] / anova_table["sum_sq"].sum()
    )
    anova_table = anova_table.drop(["Residual"])
    anova_table.sort_values(
        ["PR(>F)", "per_var_explained"], inplace=True, ascending=[True, False]
    )
    assert (
        anova_table["per_var_explained"].sum() - model.rsquared < 1e5
    ), "R-square and per_var_explained not matched"
    ...
    with open(f"{DIR_NAME}/{name}.r2", "w") as r2f:
        ...
        if unsampled_df is not None:
            try:
                print(
                    f"{r2_score(unsampled_df['tput'],model.predict(unsampled_df))}",
                    file=r2f,
                )
            except:
                pass
    ...
    print(
        anova_table.loc[
            anova_table["per_var_explained"] > 0.1, ["PR(>F)", "per_var_explained"]
        ].to_string(),
        file=f,
    )
    print("--------------------------------------------------------", file=f)
    ...
    cof_table["PR>F"] = cof_table["parms"].apply(
        lambda x: find_in_anova(anova_table, x, "PR(>F)")
    )
    # cof_table['per_var_explained'] = cof_table['parms'].apply(lambda x:anova_table.loc[x,'per_var_explained'])
    cof_table["per_var_explained"] = cof_table["parms"].apply(
        lambda x: find_in_anova(anova_table, x, "per_var_explained")
    )
    cof_table.sort_values(
        ["PR>F", "per_var_explained", "parms"],
    ...
    print("--------------------------------------------------------", file=f)
    cof_table_group = cof_table.groupby(
        ["PR>F", "parms", "per_var_explained"], dropna=False
    )
    ...
    for parm_orig in parms_list:
                        ...
                        [
                            {
                                "term": parm_orig,
                                "p-value": PRf,
                                "per_var_explained": per_var_explained,
                            }
                        ]
                    ),
                ],
        ...
        print(
            f"\nTerm: {parm}, Percentage of variance explained:{per_var_explained:0.4f}",
            file=f,
        )
        if ":" in parm:
            parm_0 = parm.split(":")[0]
            ...
            group[parm_0] = group[parm_0].apply(
                lambda x: re.sub(
                    "C\(\w+\)",
                    "",
                    x.replace("[", "").replace("]", "").replace("T.", ""),
                )
            )
            group[parm_1] = group[parm_1].apply(
                lambda x: re.sub(
                    "C\(\w+\)",
                    "",
                    x.replace("[", "").replace("]", "").replace("T.", ""),
            ...
            for idx1, group1 in group.groupby(parm_0):
                turning_coef, (spear1, spear2), turning_ratio = detect_turning_coef(
                    group1, parm_1
                )
                ...
                if turning_coef:
                    all_turning &= True
                    dff = df.loc[
                        df[trend_data["fixed_parm_name"]]
                        == trend_data["fixed_parm_val"]
                    ]
                    settings = parms.copy()
                    ...
                    if dff.groupby(by=settings).ngroups > 1:
                        ...
                        print(
                            f"Turning point: {parm_1}={turning_coef},{parm_0} fixed at{idx1}",
                            file=f,
                        )
                    ...
                    all_turning &= False
                    print(
                        f"When fixing {parm_0}@{idx1}, Overall trend: {spear_message_one(spear1)} with spearman coef = {spear1:.2f}\n",
                        file=f,
                    )
            ...
            if all([x > 0.5 for x in spear_group]):
                print(
                    f"**Overall, Coef. increase with {parm_1} while {parm_0} is fixed\n",
                    file=f,
                )

            if all([x < -0.5 for x in spear_group]):
                print(
                    f"**Overall, Coef. decrease with {parm_1} while {parm_0} is fixed\n",
                    file=f,
                )

            if all([x > -0.2 for x in spear_group]) and all(
                [x < 0.2 for x in spear_group]
            ):
                print(f"**Overall, Coef. flat while {parm_0} is fixed\n", file=f)

            if all_turning:
                print(
                    f"**Overall, Coef. all have a turning point while {parm_0} is fixed\n",
                    file=f,
                )
            ...
            for idx2, group2 in group.groupby(parm_1):
                ...
                if turning_coef:
                    ...
                    if dff.groupby(by=settings).ngroups > 1:
                        ...
                        trend_data["turning_ratio"] = turning_ratio
                        print(
                            f"Turning point: {parm_0}={turning_coef},{parm_1} fixed at {idx2}",
                            file=f,
                        )
                        print(spear_message(spear1, spear2), file=f)
                else:
                    all_turning &= False
                    print(
                        f"When fixing {parm_1}@{idx2}, Overall trend: {spear_message_one(spear1)} with spearman coef = {spear1:.2f}\n",
                        file=f,
                    )
            ...
            if all([x > 0.5 for x in spear_group]):
                print(
                    f"**Overall, Coef. increase with {parm_0} while {parm_1} is fixed\n",
                    file=f,
                )

            if all([x < -0.5 for x in spear_group]):
                print(
                    f"**Overall, Coef. decrease with {parm_0} while {parm_1} is fixed\n",
                    file=f,
                )
            ...
            group["index_value"] = group["index"].apply(
                lambda x: float(x.replace(")[", "").replace("]", "").split("T.")[1])
            )
            ...
            trend_data["spear_all"] = spearmanr(
                group["index_value"], group["Coef."]
            ).correlation
            trend_data["weight"] = 1
            ...
            if turning_coef:
                ...
                print(
                    f"Overall trend: {spear_message_one(spear1)} with spearman coef = {spear1:.2f}",
                    file=f,
                )
    ...
    try:
        df_trend["delta_spear"] = (
            df_trend["turning_spear1"] - df_trend["turning_spear2"]
        )
    ...
    if "turning_ratio" not in df_trend.columns:
        ...
        df_trend["combined_rank"] = (
            df_trend["per_var_explained"] * df_trend["turning_ratio"]
        )
    ...
    df_trend.sort_values(
        ["combined_rank", "per_var_explained", "delta_spear"],
        inplace=True,
    ...
    if not sampled:
        print(
            f"{name},{len(init_terms(parms))},{len(list(df_trend.term_raw.unique()))},{df_trend.loc[~df_trend['turning'].isna()].shape[0]}"
        )
...
def ANOVA_coef(
    ...
    if sampled:
        ...
        if any([name.startswith(sp) for sp in singles_pattern]):
            model, removed_terms, fail_to_remove, stop_at = remove_terms_single(
                parms, df
            )
        else:
            model, removed_terms, fail_to_remove, stop_at = remove_terms_combine(
                parms, df
            )
        ...
        try:
            ...
            print(
                f"Could not conduct analysis on full data set with single terms for {name}"
            )
            # raise e
        ...
        try:
            ...
            print(
                f"Could not conduct analysis on full data set with combined terms for {name}"
            )
            # raise e
    ...
    try:
        ...
        if result_dir:
            ANOVA_coef(
                ...
                name,
                DIR_NAME,
                parms_to_exclude=parms_to_exclude,
                sampled=True,
...
def compare_trend(
    ...
    for term in term_to_check:
        ...
        if term not in df2.term_raw.unique():
            ...
            df2_term = df2.loc[df2["term_raw"] == term, :]
            print(
                f"Diff in {term}'s per_var_explained: {abs(df1_term.iloc[0,2]-df2_term.iloc[0,2]):.4f}",
                file=f,
            )
            print(
                f"->Full:{df1_term.iloc[0,2]:.4f}, Sampled:{df2_term.iloc[0,2]:.4f}\n",
                file=f,
            )
            ...
            if df1_term.shape[0] == 1:
                ...
                per_diff_overall_list.append(
                    abs(df1_term.iloc[0, 5] - df2_term.iloc[0, 5])
                    / abs(df1_term.iloc[0, 5])
                )
                ...
                if np.isnan(df1_turning) and np.isnan(df2_turning):
                    ...
                    if not (
                        (
                            df1_term["turning_spear2"].iloc[0] == -888
                            and df2_term["turning_spear2"].iloc[0] == -888
                        )
                        or (
                            df1_term["turning_spear2"].iloc[0] != -888
                            and df2_term["turning_spear2"].iloc[0] != -888
                    ...
                    print(
                        f"Turning point in full dataset:{df1_turning}, spear1={df1_term['turning_spear1'].iloc[0]:.4f}, spear2={df1_term['turning_spear2'].iloc[0]:.4f}",
                        file=f,
                    )
                    print(
                        f"Turning point in sampled dataset:{df2_turning}, spear1={df2_term['turning_spear1'].iloc[0]:.4f}, spear2={df2_term['turning_spear2'].iloc[0]:.4f}",
                        file=f,
                    )
                else:
                    num_turning_points_changed += 1 / term_weight
                ...
                for (name, val), gp1 in df1_term_grp:
                    ...
                    gp2 = df2_term.loc[
                        (df2_term["fixed_parm_name"] == name)
                        & (df2_term["fixed_parm_val"] == val)
                    ]
                    if gp2.shape[0] == 0:
                    ...
                    per_diff_overall_group.append(
                        abs(gp1.iloc[0, 5] - gp2.iloc[0, 5]) / abs(gp1.iloc[0, 5])
                    )
                    ...
                    if np.isnan(df1_turning) and np.isnan(df2_turning):
                        ...
                        if not (
                            (
                                gp1["turning_spear2"].iloc[0] == -888
                                and gp2["turning_spear2"].iloc[0] == -888
                            )
                            or (
                                gp1["turning_spear2"].iloc[0] != -888
                                and gp2["turning_spear2"].iloc[0] != -888
                        ...
                        print(
                            f"    Turning point in full dataset:{df1_turning}, spear1={gp1['turning_spear1'].iloc[0]:.4f}, spear2={gp1['turning_spear2'].iloc[0]:.4f}",
                            file=f,
                        )
                        print(
                            f"    Turning point in sampled dataset:{df2_turning}, spear1={gp2['turning_spear1'].iloc[0]:.4f}, spear2={gp2['turning_spear2'].iloc[0]:.4f}",
                            file=f,
                        )
                    else:
                        num_turning_points_changed += 1 / term_weight
    ...
    if SAVE_INTERIM_FILE:
        ...
        print(
            f"***{num_of_terms_changed} terms are no longer significant in sampled dataset",
            file=f,
        )
        print(
            f"***{num_turning_points_changed} turning points/knee points mismatch",
            file=f,
        )
        print(
            f"***Mean difference in terms of per_var_explained:{np.mean(diff_per_var_list):.4f}",
            file=f,
        )
        print(f"***R-squared in full dataset:{r2_full_val}", file=f)
        print(
            f"***R-squared of per_var_explained in sampled dataset:{r2_sampled_val}",
            file=f,
        )

        print(
            f"***Mean difference in terms of overall spearmen rank correlation: {np.mean(diff_overall_list):.4f} ",
            file=f,
        )
        print(
            "====================================================================================",
...
def calc_frechet(
    full_cof_csv, sample_cof_csv, name, out_dir, empty_d2=False, stratified_term=False
):
...
def run_one_comparison(sample_csv_path, sample_method, write_lock=None):
    ...
    trend1 = compare_trend(
        ...
        os.path.join(result_dir_sampled, f"{exp_name}___trend.csv"),
        exp_name,
        result_dir_sampled,
        empty_d2=empty_d2,
    )
    trend2 = calc_frechet(
        os.path.join(result_dir_full, f"{exp_name_full}___cof.csv"),
        os.path.join(result_dir_sampled, f"{exp_name}___cof.csv"),
        exp_name,
        result_dir_sampled,
        empty_d2=empty_d2,
    )
    ...
    with write_lock:
        with open(f"{OUT_BASE_DIR}/RESULT_{sample_method}.csv", "a") as f:
            if sample_method == "stratified"  or sample_method =="neyman" :
                ...
                print(
                    f"{info['sample']},{info['random']},{info['split']},{trend1},{trend2}",
                    file=f,
                )
...
def run_all_comparsion(sample_method):
    ...
    for work_list in chunker(csvs_path, os.cpu_count()):
        ...
        processes = [
            mp.Process(target=run_one_comparison, args=(csv, sample_method, lock))
            for csv in work_list
        ]
...
def run_calvin_comparison():
    ...
    random_sampling(
        original_data,
        f"{output_dir}/method@random_sys@calvin-Calvin-comp_bench@tpcc",
        seed=None,
    )
    random_sampling(
        revised_data,
        f"{output_dir}/method@random_sys@aria-Calvin-comp_bench@tpcc",
        seed=None,
    )
